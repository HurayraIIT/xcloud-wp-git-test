...
class Helper
{
    ...
    public static function insert_new_category( $slug ) {
        if( !!intval($slug) ) return $slug;
        
        $is_cat_exists = \BetterLinks\Helper::term_exists( $slug );
        if( !$is_cat_exists ) {
            $insert_id = \BetterLinks\Traits\Query::insert_term([
                'term_name' => $slug,
    ...
    public function add_password_protect_page() {
        if( $this->get_page_title_for_slug('password-protected-form') ) return false;
