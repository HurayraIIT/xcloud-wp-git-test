...
class Link
{
    ...
    public function before_redirect($data)
    {
        ...
        if( class_exists('\BetterLinksPro\Helper') ) {
            $password = \BetterLinksPro\Helper::get_password_by_link_id($data['ID']);
            
            if( !empty(self::$link_options['enable_password_protection']) && count($password) > 0) {
                $data['is_protected'] = '1' === $password[0]->status;
            }
        }

        if( $data['is_protected'] ) {
            wp_redirect(esc_url_raw(site_url().'/password-protected-form/?short_url='.esc_attr($data['short_url'])));
            exit();
        }
    }
    ...
    public function start_external_analytics_cron_job($data)
    {
        ...
        if($ga_tracking_code){
            wp_schedule_single_event(time(), 'betterlinkspro/send_google_analytics_data', array($data, $_SERVER['REQUEST_URI'], $ga_tracking_code, $ga4_api_secret));
        }
    ...
    {
        if (isset($data['link_status']) && ($data['link_status'] === 'draft' || $data['link_status'] === 'scheduled')) {
            return false;
        }
        return true;
    }
    ...
    public static function get_password_protection_status() {
        $settings = self::$link_options;
        return !empty($settings['password']) ? $settings['password'] : null;
    }
}